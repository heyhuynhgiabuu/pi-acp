...
The upstream RuleTester cases and full JavaScript/TypeScript test suites were not copied into this repository. The root `tsconfig.json` also does not typecheck vendored plugin files; `npm run lint` loads the plugin through Oxlint, but this is not a claim of full upstream test conformance.
